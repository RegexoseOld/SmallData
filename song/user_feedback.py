...
class Tonality:
    ...
    def calculate_rack_values(self, cat):
        ...
        if sum(self.tonality_counter.values()) > self.min_utts:
            ...
            if not self.tonality_lock:
                ...
                self.most_common = self.tonality_counter.most_common(1)
        ...
        self.last_value = self.chain_controls[self.FX_KEY][1] # restore last value of that chain
        self.ctrl_val = self.last_value + self.category_to_chain[cat][1]  # make new actual ctrl_value
        self.chain_controls[self.FX_KEY][1] = self.ctrl_val   # update chain_controls
    ...
    def reset_tonality(self):
        for v in self.category_to_chain.values():
            init_value = v[1]
            chain_key = v[0]
            self.chain_controls[chain_key][1]= init_value
