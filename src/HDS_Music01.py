...
from HDS_Client1 import START
from HDS_rules import normal_values
...
class Edit:
    ...
    def update_controllers(self, cl_map):
        ...
        if cl_map['cc_dict'] == None:
            print('ende?: {}'.format(cl_map))
            playthread.slot = 14
            playthread.jump(14)
            ...
            print('coarse: {}\ntrigger: {}\n'.format(coarse, trigger))
            cc_map = cl_map['cc_dict']
            for key in list(cc_map.keys()):
                print(key, ' : ', cc_map[key], '\n')
            ...
            if (position + RULES[trigger]) >= 15:
                ...
                print('trigger direkt: ', type(trigger))
                playthread.slot = int(trigger)
...
class Play_Thread(threading.Thread):
    ...
    def run(self):
        ...
        while self.play == True:
            ...
            print('run slot: {} play: {}'.format(self.slot, play))
...
if __name__ == "__main__":
    ...
    def make_slots(slotlist):
        for slot in range(len(slotlist)):
            note_obj = make_note_objects(slot+1, [1])
            play = Play(note_obj)
            play.cc_map = normal_values
            name = 'slot{}'.format(slot+1)
            play_dict[name] = play

    slotlist = [x for x in range(17)]
    make_slots(slotlist)
