...
class Edit:
    ...
    def update_controllers(self, cl_map):
        ...
        if (position + RULES[trigger]) >= 25:
            ...
            PLAYTHREADS['t1'].slot = 25


        elif trigger in [ "+1", "-1", "+2", "-2", '+3', '+4', '+10']:
            PLAYTHREADS['t1'].slot += RULES[trigger]
            PLAYTHREADS['t1'].send_ccvals(normal_values)
            print('genullt?: ', PLAYTHREADS['t1'].ccmap == normal_values)
...
class ClientIO(SimpleUDPClient):
    ...
    def feedback_for_client(self, P_Obj, repeat):
        ...
        print('feedback for client: {}, time: {}'.format(dict_to_send, (time.time()- START)))
...
if __name__ == "__main__":
    ...
    slotlist = [x for x in range(1, songs[args.song])]
    make_slots(slotlist)
