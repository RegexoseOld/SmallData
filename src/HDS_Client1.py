...
DATETIME = datetime(2018, 9, 3, 10, 31, 30, 11111)
...
file1 = Image.open('../../Grafiken/Muster/dots.gif')
file2 = Image.open('../../Grafiken/Muster/star.gif')
file3 = Image.open('../../Grafiken/Muster/marmor.gif')
file4 = Image.open('../../Grafiken/Muster/check.gif')
...
class Interpreter:
    ...
    def similar_word(self, word):
        similar = self.classifier.get_similar_words(word)
...
class TextOutput(tk.Frame):
    ...
    HEADLINE = ("Verdana Bold", 50)
    LARGE_FONT = ("Verdana Bold", 40)
    ...
    def __init__(self, parent, controller):
        ...
        self.form = self.clas_can.create_image(400, 200, image=self.image, tag='form')
        self.classified = self.clas_can.create_text(*self.clastext_coords,
                                                    text='',
                                                    fill='black', tag='clas', font=TextOutput.LARGE_FONT,
                                                    width=1800, anchor=tk.NW)
        self.after_id = None
        #self.song_position()


    def read_ref(self, ref):
        index = random.randint(1, len(ref)) - 1
        print('reflen: {}  index: {}'.format(len(ref), index))
        line = ref.pop(index)
        user, intent, text = line.split('|')
        time = len(text) * 27
        if ref:
            self.textedit.feedback.dict_comp(user, intent, text)
            self.after(time, self.read_ref, ref)
            self.reftext = text
            self.bell()
        else:
            self.textedit.feedback.dict_comp('Programm', 'Prozess', 'ENDE')
    ...
    def image_var(self, cat):
        ...
        self.clas_can.itemconfigure('form', image=self.image)
        # self.clas_can.configure(background=CAT2COLOR[cat])

    def createCanvas(self):
        self.clas_can = tk.Canvas(self, width=1800, height=400, borderwidth=5)
        # self.scroll = tk.Scrollbar(self, orient='vertical', command=self.clas_can.yview)
        self.clas_can.grid(column=0, row=1, columnspan=3, padx=5, sticky="NSWE")
    ...
    def clastext_update(self, user, cat, text):
        insert = "User: {}, Kategorie: {}\n{}\n".format(user, cat, text)

        # self.clas_can.insert('clas', tk.END, insert)
        self.clas_can.itemconfigure('clas', text=insert)
        self.image_var(cat)
    ...
    def createLabels(self):
        ...
        self.label04 = tk.Label(self, text="Kooperationsvereinbarung Haus der Statistik", font=TextOutput.HEADLINE)
    ...
    def category_stats(self, fig, stats):
        ...
        # schriftgröße?
        a.set_aspect('equal')
        # a.legend(bbox_to_anchor=(1, 0, 0.2, 1))
        # print('cat_stats: ', cats)
        return fig
    ...
    def plot_stats(self, stats):
        # print('plotstats update: ', stats)

        fig = Figure(figsize=(5,6), dpi=128)
        ...
        canvas._tkcanvas.grid(column=0, row=2, columnspan=3, padx=10, sticky="NSEW")
